...
class _PostFoundItemPageState extends State<PostFoundItemPage> {
  ...
  void sendNotifications(String foundCategory, String foundLocation,
      String foundItemID) async {
    // Step 1: Retrieve data from Lost Product database
    QuerySnapshot lostProducts = await FirebaseFirestore.instance
        .collection('LostProduct')
        .where('Category', isEqualTo: foundCategory)
        .where('ItemLocation', isEqualTo: foundLocation)
        .where('IsRetrieved', isEqualTo: false) // Include only unretrieved lost items
        .get();

    // Step 2: Filter users
    List<String> looserUserIDs = [];
    for (QueryDocumentSnapshot lostProduct in lostProducts.docs) {
      String looserID = lostProduct['UserID'];
      if (looserID != foundItemID) {
        looserUserIDs.add(looserID);
      }
    }

    // Step 3: Retrieve FCM tokens
    List<String> fcmTokens = [];
    for (String looserID in looserUserIDs) {
    ...
    for (String fcmToken in fcmTokens) {
      await FirebaseMessaging.instance.sendMessage(
        ...
        notification: RemoteNotification(
          ...
          body: 'A similar item to what you lost has been found nearby!',
        ),
      );
    }
  }
