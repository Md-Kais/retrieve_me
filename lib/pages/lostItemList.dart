...
class _LostItemListPageState extends State<LostItemListPage> {
  ...
  Future<void> addLostProductToUser(String userId, String productId, String postUserID) async {
    try {
      ...
      if (userSnapshot.exists) {
        ...
        List<String> lostProductIds = List<String>.from(userData['lostProductIds'] ?? []);

        // Add the new lost product ID to the list
        lostProductIds.add(productId);

        // Retrieve the existing list of message product IDs or create an empty list
        List<String> messageProductIds = List<String>.from(userData['messageProductIds'] ?? []);
